<section class="space-y-6">
  <nav class="breadcrumb-card px-4 py-3 text-sm" aria-label="Breadcrumb">
    <ol class="flex flex-wrap items-center gap-2 text-gray-600">
      ...
      <li class="font-bold text-gray-900" aria-current="page">{{ $category->name ?? 'Product Listing' }}</li>
    </ol>
  </nav>
  <div>
    <div class="mb-5 flex flex-col gap-3 rounded-lg border border-gray-200 bg-white p-5 md:flex-row md:items-center md:justify-between">
      <div>
        ...
        <h1 class="text-3xl font-black">{{ $category->name ?? 'Product Listing' }}</h1>
        <p class="text-sm text-gray-500 mt-1">{{ $products->count() }} products found</p>
      </div>
      <div class="flex items-center justify-between sm:justify-end gap-3">
        <div class="hidden sm:inline-flex rounded-md border border-gray-300 bg-white p-0.5">
          <button class="view-toggle rounded px-2.5 py-1.5 text-xs sm:text-sm font-black active" type="button" data-view-toggle="grid" data-target="category-products" aria-pressed="true">
          ...
          <button class="view-toggle rounded px-2.5 py-1.5 text-xs sm:text-sm font-black" type="button" data-view-toggle="list" data-target="category-products" aria-pressed="false">
            <svg class="w-4 h-4 sm:w-5 sm:h-5 inline mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <article class="product-card relative group bg-white rounded-lg sm:rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 h-full flex flex-col">
          ...
          <div class="product-info flex flex-col flex-1 p-2 sm:p-3">
            <a href="{{ url('product/'.$product->slug) }}" class="card-link block">
              <h3 class="font-bold text-gray-800 hover:text-primary transition-colors line-clamp-2 text-sm sm:text-base">{{ $product->name }}</h3>
            </a>
            <div class="price-container mt-auto flex flex-wrap items-center justify-between gap-2 pt-2">
              ...
              <div class="flex items-center gap-1 sm:gap-1">
                <a class="btn-go flex items-center justify-center transition-transform hover:scale-105 bg-gray-100 text-gray-700 hover:bg-primary hover:text-white rounded-md px-1 py-1 sm:px-2 sm:py-1 text-xs sm:text-xs" href="{{ url('product/'.$product->slug) }}">
                <i class="fa-solid fa-magnifying-glass-plus sm:mr-1"></i>
                <span class="hidden sm:inline">View</span>
                </a>
                <a class="btn-go flex items-center justify-center transition-transform hover:scale-105 bg-gray-100 text-gray-700 hover:bg-primary hover:text-white rounded-md px-1.5 py-1 sm:px-2 sm:py-1 text-xs sm:text-xs" href="{{ url('#') }}">
