...
@section('content')
  ...
  <div class="carousel col-span-full w-full mx-auto" aria-roledescription="carousel">
    <div class="carousel-track" aria-live="polite">
      ...
      @empty
        <a href="{{ url('listing.html') }}" class="carousel-slide" role="group" aria-roledescription="slide" aria-label="1 of 1" style="background-image: url('https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=1600&q=80&auto=format&fit=crop');">
        </a>
      @endforelse
    </div>
    ...
    <div class="carousel-dots" role="tablist" aria-label="Carousel dots">
      @foreach($sliderBanners as $index => $banner)
        <button class="carousel-dot {{ $loop->first ? 'active' : '' }}" role="tab" aria-selected="{{ $loop->first ? 'true' : 'false' }}"></button>
      @endforeach
    </div>
  </div>
